...
const ModeratorDashboard = () => {
    ...
    const token = sessionStorage.getItem('moderator_auth');
    ...
    const fetchSubmissions = async (sessionId) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/moderator/sessions/${sessionId}/submissions`, {
                headers: { 'Authorization': `Basic ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
    ...
    const handleSaveGrade = async (submissionId, formData) => {
        const res = await fetch(`/api/moderator/submissions/${submissionId}/grade`, {
            method: 'PUT',
            headers: {
    ...
    const filteredSessions = sessions.filter(s => {
        ...
        // In the SessionResponse, assignmentTitle is formatted as "[ModuleName] AssignmentTitle"
        // Or we can check if it starts with the module's name
                                        ...
                                        <tbody>
                                            {submissions.map(sub => (
                                                <tr key={sub.submissionId} className="border-b border-slate-800/50 hover:bg-slate-850/30 transition-colors">
                                                    ...
                                                    <td className="py-3 px-4 font-bold text-emerald-400">{sub.grade !== null ? `${sub.grade}%` : '-'}</td>
                                                    <td className="py-3 px-4 text-right">
                                                        <button
                                                            onClick={() => setActiveSubmission(sub)}
                                                            className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-[10px] py-1.5 px-3 rounded-lg transition-colors cursor-pointer"
                                                        >
                                                            Inspect & Mark
                                                        </button>
                                                    </td>
